...
const Music = require('../musics/music.model');
...
router.get("/", async (req, res) => {
    try {
        ...
        // 4. Trending musics statistics: 
        const trendingMusicsCount = await Music.aggregate([
            { $match: { trending: true } },  // Match only trending books
            { $count: "trendingMusicsCount" }  // Return the count of trending musics
        ]);
        
        // If you want just the count as a number, you can extract it like this:
        const trendingMusics = trendingMusicsCount.length > 0 ? trendingMusicsCount[0].trendingMusicsCount : 0;
